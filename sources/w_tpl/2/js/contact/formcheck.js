...
var FormCheck = new Class({
	...
	validate : function(el) {
		...
		el.validation.each(function(rule) {
			if(this.isChildType(el)) {
				...
				if (ruleMethod == 'userdupname') {
					if (this.validateUserDupName(el) == false) {
						el.isOk = false;
					}
				}
				...
				if(ruleMethod=='notTitle'){
					if (this.validateNotTitle(el, ruleArgs) == false) {
						el.isOk=false;
					}
				}				
				if (ruleMethod == 'words') {
	...
	*/
	...
	validateUserDupName: function(el){
		var unique=false;
		var _this=this;
		var res=new Request.HTML({
			method: 'get',
			url:'./index.php?main=checkuser&ajax=1&username='+el.value,
			async:false,							
			link:'ichain',			
			onComplete: function(){
				if(this.response.text=='sliska_sprawa'){
					var msg = _this.options.alerts.userdupname;
					el.errors.push(msg);
					unique=false;
				}
				else{
	...
	*/
	...
	validateCaptcha: function(el){
		...
		var res=new Request.HTML({
			method: 'get',
			async: false,
			url:'./php/check.php?captcha='+el.value,
			link:'ichain',			
			onComplete: function(){
				if(this.response.text=='jak_po_masle'){
					...
					catpchaok=false;
					var msg = _this.options.alerts.captcha;
					el.errors.push(msg);
				}
			}
		}).send();
